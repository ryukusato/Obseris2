...
class BoardRenderer:
    ...
    def _draw_clear_flash(self, before, after, ox, oy):
        """
        消える行に含まれる全ブロックを白く描画する
        """
        bright = (245, 245, 245)  # 完全白より少し抑えると見やすい

        for y in range(self.board_h):
            # 消える行判定
            full_before = all(before[y][x] for x in range(self.board_w))
            any_after_zero = any(after[y][x] == 0 for x in range(self.board_w))

            if full_before and any_after_zero:
                for x in range(self.board_w):
                    pygame.draw.rect(
                        self.screen,
                        bright,
...
def clear_text(landing):
        ...
        name = str(landing.kind).split(".")[-1]

        if landing.used_t_piece:
            name = "T-" + name

        if landing.back_to_back:
            name += " (B2B)"
